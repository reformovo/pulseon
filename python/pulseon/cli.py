...
def _run(client: _pulseon.Client, args: argparse.Namespace) -> str:
    ...
    if args.action == "list":
        ...
        return _render_summaries(metrics, include_metric_key=True)
    ...
    return _render_summaries(summaries, include_metric_key=False)


def _render_summaries(
    summaries: Sequence[_pulseon.MetricSummary], *, include_metric_key: bool
) -> str:
    headers = ["RUN_ID"]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        with _pulseon.init(pathlib.Path.cwd(), _must_exist=True) as client:
